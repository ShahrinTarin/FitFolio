      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-12">
        {posts.map((post, index) => (
          <motion.div
            ...
            className="bg-[#0f0f0f] rounded-3xl p-6 border border-lime-500/20 shadow-[0_0_14px_#84cc16] hover:shadow-[0_0_40px_#84cc16] hover:scale-[1.03] transition duration-300 ease-in-out overflow-hidden"
          >
            {post.image && (
              <motion.img
                src={post.image}
            ...
            <h3 className="text-xl font-bold text-lime-400 mb-2 line-clamp-2">
              <Link to={`/forums/${post._id}`} className="hover:underline">
                {post.title}
              </Link>
            </h3>
            ...
            <span className="inline-block bg-lime-600/20 text-lime-400 px-3 py-1 text-xs font-medium rounded-full">
              {post.category}
            </span>
          </motion.div>
        ))}
